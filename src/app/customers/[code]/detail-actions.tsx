...
export function DetailActions({ c }: Props) {
  ...
  const canDelete = c.cases.length === 0 && c.machines.length === 0;

  const onDelete = () => {
    setDelError(null);
    startTransition(async () => {
      const r = await deleteCustomer(c.code);
      if (!r.success) {
  ...
  return (
    <>
      <div style={{ display: "flex", gap: 8 }}>
        ...
        >
          <Icon name="wrench" size={14} /> Edit customer
        </button>
        {canDelete && (
          confirmDelete ? (
      ...
      <EditCustomerSheet open={editOpen} onClose={() => setEditOpen(false)} c={c} />
    </>
  );
}
